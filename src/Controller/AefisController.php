...
class AefisController extends AppController
{
    ...
    public function edit($id = null)
    {
        ...
        $aefi = $this->Aefis->get($id, [
            'contain' => ['AefiListOfVaccines', 'Attachments', 'ReportStages', 'AefiReactions'],
        ...
        if ($this->request->is(['patch', 'post', 'put'])) {
            $aefi = $this->Aefis->patchEntity($aefi, $this->request->getData(), [
                ...
                'associated' => [
                    'AefiReactions' => ['validate' => ($this->request->getData('submitted') == 2) ? true : false],
            ...
            if ($aefi->submitted == 1) {
                ...
                if ($this->Aefis->save($aefi)) {
                    ...
                    $data['vars']['pdf_link'] = $html->link('Download', [
                        'controller' => 'Aefis', 'action' => 'view', $aefi->id, '_ext' => 'pdf',
                        '_full' => true
                    ]);
        ...
        //  $d = $this->Meddras->find('list', ['limit' => 500, 'fields' => array('Meddras.terminology'), 'order' => 'Meddras.terminology'])->toArray();
        // $pp = json_encode(array($d));
        //  dd($d);

        //  $adverse = $this->Meddras->find('list', array('limit' => 200, 'fields' => array('Meddras.id', 'Meddras.terminology')));
    ...
    public function view($id = null)
    {
        ...
        $designations = $this->Aefis->Designations->find('list', array('order' => 'Designations.name ASC'));
        $provinces = $this->Aefis->Provinces->find('list', ['limit' => 200]);
        $this->set(compact('aefi', 'designations', 'provinces', 'adverse_events'));
        $this->set('_serialize', ['aefi', 'designations', 'provinces', 'adverse_events']);
    }
    ...
    public function followup($id = null, $fid = null)
    {
        ...
        $designations = $this->Aefis->Designations->find('list', array('order' => 'Designations.name ASC'));
        $provinces = $this->Aefis->Provinces->find('list', ['limit' => 200]);
        $adverse_events = [1, 1, 2, 3, 4];
        $this->set(compact('aefi', 'designations', 'provinces', 'followup', 'adverse_events'));
        $this->set('_serialize', ['aefi']);
    }
