...
class AefisController extends AefisBaseController
{
    ...
    public function assignSelf()
    {
        ...
        if (isset($aefi->id) && $this->request->is('post')) {
            ...
            $aefi->action_date = date("Y-m-d H:i:s"); //Updated report action date
            $aefi->status = 'Assigned';
    ...
    public function changeEvaluator() {
        $aefi = $this->Aefis->get($this->request->getData('id'), ['contain' => 'ReportStages']);
        if (isset($aefi->id) && $this->request->is('post')) {
            $aefi->assigned_by = $this->Auth->user('id');
            $aefi->assigned_to = $this->request->getData('assigned_to');
            $aefi->assigned_date = date("Y-m-d H:i:s");
            $aefi->action_date = date("Y-m-d H:i:s"); //Updated report action date
            $evaluator = $this->Aefis->Users->get($this->request->getData('assigned_to'));
            ...
            if ($this->Aefis->save($aefi)) {
                ...
                if($this->request->is('json')) {
                    $this->set([
                        'message' => 'Evaluator '.$evaluator->name.' assigned ADR '.$aefi->reference_number, 
                        '_serialize' => ['message']]);
                    return;
                }
