...
class SaefisController extends AppController
{
    ...
    public function followup($id = null)
    {
        ...
        $followup = $this->SaefiFollowups->newEntity() ;

        if(!empty($saefi) && $saefi->user_id == $this->Auth->user('id')) {
            if ($this->request->is(['patch', 'post', 'put'])) { 
                $followup = $this->SaefiFollowups->patchEntity($followup, $this->request->getData());
                
                $followup->report_type = 'FollowUp';
                $followup->messageid = null;
                $followup->submitted_date = date("Y-m-d H:i:s");
                $followup->submitted = 2;
                $followup->saefi_id = $saefi->id;
                //Attachments
                $this->_attachments();
                if (!empty($followup->attachments)) {
                    for ($i = 0; $i <= count($followup->attachments)-1; $i++) { 
                      $followup->attachments[$i]->model = 'SaefiFollowups';
                      $followup->attachments[$i]->category = 'attachments';
                    }
                }
                //submit to mcaz button
                ...
                //TODO: validate linked data here since validate will be false
                if ($this->SaefiFollowups->save($followup, ['validate' => false])) {

                    //send email and notification
                    $this->loadModel('Queue.QueuedJobs');    
                    $data = [
                          'email_address' => $saefi->reporter_email, 'user_id' => $this->Auth->user('id'),
                          'type' => 'applicant_submit_saefi_followup_email', 'model' => 'Saefis', 'foreign_key' => $saefi->id,
                          'vars' =>  $saefi->toArray()
                    ];
                    //notify applicant
                    $this->QueuedJobs->createJob('GenericEmail', $data);
                    $data['type'] = 'applicant_submit_saefi_followup_notification';
                    $data['vars']['created'] = $followup->created;
                    $this->QueuedJobs->createJob('GenericNotification', $data);
                    //notify managers
                    $managers = $this->Saefis->Users->find('all')->where(['group_id IN' => [2, 4]]);
                    foreach ($managers as $manager) {
                        $data = ['email_address' => $manager->email, 'user_id' => $manager->id, 'model' => 'Saefis', 'foreign_key' => $saefi->id,
                                 'vars' =>  $saefi->toArray()];
                        $data['type'] = 'manager_submit_saefi_followup_email';
                        $this->QueuedJobs->createJob('GenericEmail', $data);
                        $data['type'] = 'manager_submit_saefi_followup_notification';
                        $this->QueuedJobs->createJob('GenericNotification', $data);
                    }
                    //
                     $this->set(compact('followup'));
