...
echo $this->Form->control('aefi_pr_id', ['type' => 'hidden', 'value' => $aefi->id, 'escape' => false, 'templates' => 'table_form']);
        ...
        <div class="form-group">
            <div class="col-sm-offset-4 col-sm-8">
                <button type="submit" class="btn btn-primary active" id="registerUser"><i class="fa fa-share-square-o"
                        aria-hidden="true"></i>
                    Assign</button> OR <button type="submit" class="btn btn-primary active" id="registerUser"><i
    ...
    ?>
        ...
        <h4 class="text-center">Assigned to: <span
                <?php if (!in_array("Evaluated", Hash::extract($aefi->report_stages, '{n}.stage'))) {?> id="assigned-to"
                class="editable" data-type="select" data-pk="<?=$aefi->id?>"
                ...
                <?=$this->cell('Signature::index', [$aefi->assigned_to])?></span> on <?=$aefi->assigned_date?></h4>
        <?php
}
?>
    </div>
...
?>
        ...
        <hr>
        <?php if (!empty($aefi->assigned_to) && $prefix == 'manager' && $aefi->assigned_to !=($this->request->session()->read('Auth.User.id')) ) {
    ?>
        ...
        <div class="row">
            <div class="col-xs-12">
                <h5 class="text-center">Send request to evaluator
                    <?=$this->cell('Signature::index', [$aefi->assigned_to])?> for more information</h5>
            </div>
            <div class="col-xs-12">
                <?php
echo $this->Form->control('aefi_pr_id', ['type' => 'hidden', 'value' => $aefi->id, 'escape' => false, 'templates' => 'table_form']);
