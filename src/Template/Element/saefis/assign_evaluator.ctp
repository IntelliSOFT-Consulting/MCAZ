...
?>
  <div class="row">
    <div class="col-xs-12">
      <?php 
        if(empty($saefi->assigned_to)) {
          ...
          <div class="row">
            <div class="col-xs-6">                
                <?php echo $this->Form->create($saefi, ['url' => ['action' => 'assign-evaluator']]) ?>
            ...
            <div class="col-xs-6">
            <?php echo $this->Form->create($saefi, ['url' => ['action' => 'assign-self']]) ?>
         ...
         ?>
          ...
          <?php if(!in_array("Evaluated", Hash::extract($saefi->report_stages, '{n}.stage'))) { ?>
            ...
            data-type="select" data-pk="<?= $saefi->id ?>"
            data-url="<?= $this->Url->build(['controller' => 'Saefis', 'action' => 'changeEvaluator',  'prefix' => 'manager', $saefi->id, '_ext' => 'json']); ?>" 
            data-name="assigned_to"
            data-title="Change evaluator"
          <?php } ?>
          class='text-center'> <?= $this->cell('Signature::index', [$saefi->assigned_to]) ?></span> on <?= $saefi->assigned_date ?></h4>
      <?php
        	}
     	?>
    </div>
    ...
    <div class="col-xs-12">
      <div class="col-xs-12">
      <?php
        if(!empty($saefi['request_evaluators']) && $prefix == 'manager') {
          $i = 0;
      ...
      <hr>
      <?php if(!empty($saefi->assigned_to) && $prefix == 'manager') { 
        ?>
          <?php echo $this->Form->create($saefi, ['url' => ['action' => 'request-evaluator']]);
                //$i = count($saefi['request_evaluators']);
           ?>
            <div class="row">
              <div class="col-xs-12"><h5 class="text-center">Send request to evaluator <?= $this->cell('Signature::index', [$saefi->assigned_to]) ?> for more information</h5></div>
              <div class="col-xs-12">
              <?php
                    echo $this->Form->control('saefi_pr_id', ['type' => 'hidden', 'value' => $saefi->id, 'escape' => false, 'templates' => 'table_form']);
