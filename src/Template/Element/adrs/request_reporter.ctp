  <div class="row">
    <div class="col-xs-12">
      <?php
      if(!empty($adr['request_reporters'])) {
        $i = 0;
        foreach ($adr['request_reporters'] as $request_reporter) {
          $i = $i+1;
    ...
    <div class="col-xs-12">
      <hr>
          <?php echo $this->Form->create($adr, ['url' => ['action' => 'request-reporter']]);
                // $i = count($adr['request_reporters']);
           ?>
            <div class="row">
              <div class="col-xs-12"><h5 class="text-center">Send request to reporter for more information</h5></div>
              <div class="col-xs-12">
	          	<?php
                    echo $this->Form->control('adr_pk_id', ['type' => 'hidden', 'value' => $adr->id, 'escape' => false, 'templates' => 'table_form']);
