...
?>
  <div class="row">
    <div class="col-xs-12">
      <?php 
        ...
        <div class="row">
            ...
            <div class="col-xs-6">
            <?php echo $this->Form->create($adr, ['url' => ['action' => 'assign-self']]) ?>
        ...
        ?>
          ...
          <?php } ?>
          class='text-center'> <?= $this->cell('Signature::index', [$adr->assigned_to]) ?></span> on <?= $adr->assigned_date ?></h4>
      <?php
        	}
     	?>
    </div>
    ...
    <div class="col-xs-12">
      ...
      <hr>
      <?php if(!empty($adr->assigned_to)  && $prefix == 'manager') { 
        ?>
          <?php echo $this->Form->create($adr, ['url' => ['action' => 'request-evaluator']]);
                //$i = count($adr['request_evaluators']);
           ?>
            <div class="row">
              <div class="col-xs-12"><h5 class="text-center">Send request to evaluator <?= $this->cell('Signature::index', [$adr->assigned_to]) ?> for more information</h5></div>
              <div class="col-xs-12">
              <?php
                    echo $this->Form->control('adr_pr_id', ['type' => 'hidden', 'value' => $adr->id, 'escape' => false, 'templates' => 'table_form']);
