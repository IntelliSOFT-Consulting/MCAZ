...
<div class="row">
  <div class="col-xs-12">
    ...
    if (empty($sadr->assigned_to)) {
      if ($prefix == 'manager') { ?>
        <div class="row">
          <div class="col-xs-6">
            ...
            <div class="form-group">
              <div class="col-sm-offset-4 col-sm-8">
                <button type="submit" class="btn btn-primary active" id="registerUser">
                  <i class="fa fa-share-square-o" aria-hidden="true"></i>Assign
                </button>
              </div>
            </div>
          ...
          <div class="col-xs-6">
            ...
            <div class="form-group">
              <div class="col-sm-offset-4 col-sm-8">
                <button type="submit" class="btn btn-success active" id="registerUser">
                  <i class="fa fa-user-o" aria-hidden="true"></i>Assign Self
                </button>
              </div>
            </div>
            <?php echo $this->Form->end() ?>
      ...
      ?>
      <br><br>
      <h4 class="text-center">Assigned to: <span <?php if (!in_array("Evaluated", Hash::extract($sadr->report_stages, '{n}.stage'))) { ?> id="assigned-to" class="editable" data-type="select" data-pk="<?= $sadr->id ?>" data-url="<?= $this->Url->build(['controller' => 'Sadrs', 'action' => 'changeEvaluator',  'prefix' => 'manager', $sadr->id, '_ext' => 'json']); ?>" data-name="assigned_to" data-title="Change evaluator" <?php } ?> class='text-center'> <?= $this->cell('Signature::index', [$sadr->assigned_to]) ?></span> on <?= $sadr->assigned_date ?></h4>
    <?php
    }
    ?>
  </div>
  ...
  <div class="row">
    ...
    <?php if (!empty($sadr->assigned_to)  && $prefix == 'manager') { ?>
      ...
      <div class="row">
        <div class="col-xs-12">
          <h5 class="text-center">Send request to evaluator <?= $this->cell('Signature::index', [$sadr->assigned_to]) ?> for more information</h5>
        </div>
        <div class="col-xs-12">
          <?php
          echo $this->Form->control('sadr_pr_id', ['type' => 'hidden', 'value' => $sadr->id, 'escape' => false, 'templates' => 'table_form']);
