...
Assign</button>
         ...
         <?php     
          ...
          <?php if(!in_array("Evaluated", Hash::extract($sadr->report_stages, '{n}.stage'))) { ?>
            id="assigned-to" class="editable"
            data-type="select" data-pk="<?= $sadr->id ?>"
          ...
          <?php } ?>
          class='text-center'> <?= $this->cell('Signature::index', [$sadr->assigned_to]) ?></span> on <?= $sadr->assigned_date ?></h4>
      <?php
        	}
     	?>
    </div>
    ...
    <div class="row">
      ...
      <hr>
          <?php echo $this->Form->create($sadr, ['url' => ['action' => 'request-evaluator']]);
           ...
           ?>
            <div class="row">
              <div class="col-xs-12"><h5 class="text-center">Send request to evaluator <?= $this->cell('Signature::index', [$sadr->assigned_to]) ?> for more information</h5></div>
              <div class="col-xs-12">
              <?php
                    echo $this->Form->control('sadr_pr_id', ['type' => 'hidden', 'value' => $sadr->id, 'escape' => false, 'templates' => 'table_form']);
