...
if (empty($aefi->assigned_to)) {
    ...
    /*
     ***
    @ Japheth, Check if assigned evaluator happens to be the current manager
     */
    $current = $this->request->session()->read('Auth.User.id');
    if ($aefi->assigned_to==$current) {
        echo "Assigned to: Self (" . $evaluators->toArray()[$aefi->assigned_to] . ")";
    }else{
