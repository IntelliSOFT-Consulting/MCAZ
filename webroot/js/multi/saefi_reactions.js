$(function () {
    ...
    function addReactionDetail() {
        console.log("clicked");
        var se = $("#saefi_reactions .reaction-group")
            .last()
            .find("button")
            .attr("id");
        ...
        if ($("#saefi_reactions .reaction-group").length < 9) {
            var new_reactiondetail = $(
                '<div class="reaction-group"> \
                        <input class="form-control" name="saefi_reactions[{i}][id]" id="saefi_reactions-{i}-id" type="hidden"> \
                        <div class="input text"> \
                            <div class="form-group"> \
                                <div class="col-xs-4 control-label"> \
                                </div> \
                                <div class="col-xs-6"> \
                                    <input class="form-control other_reactions" name="saefi_reactions[{i}][reaction_name]" maxlength="255" id="saefi_reactions-{i}-reaction-name" value="" type="text"> \
                                </div> \
            ...
            $("#saefi_reactions").append(new_reactiondetail);
        } else {
            alert(
                "Sorry, cant add more than " +
                    $("#saefi_reactions .reaction-group").length +
                    " Reactions!"
            );
        }
    }
    function removeReactionDetail() {
        ...
        var inputVal = $("#saefi_reactions-" + intId + "-id").val();
        if (inputVal) {
            $.ajax({
                type: "POST",
                url: "/saefi_reactions/delete/" + inputVal + ".json",
                data: { id: inputVal },
